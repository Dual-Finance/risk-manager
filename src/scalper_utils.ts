...
import {
  OPENBOOK_PROD_PK, MS_PER_YEAR, NO_FAIR_VALUE, OPTION_VAULT_PK, RISK_MANAGER_PK, SUFFICIENT_BOOK_DEPTH,
} from './constants';
...
export async function getSpotDelta(connection: Connection, symbol: SYMBOL) {
  ...
  let spotDelta = 0;
  let tokenDecimals = 1;
  const accountList = [OPENBOOK_PROD_PK, OPTION_VAULT_PK, RISK_MANAGER_PK];
